...
EXIT_BUTTON = 9
RECORD_BUTTON = 8
EXPERT_AGENT_BUTTON_EMERGENCY = 6
EXPERT_AGENT_BUTTON_LOCAL_MINIMUM = 7
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.Renderer = getattr(renderer_module, "Renderer")
    ...
    def restart(self):
        ...
        self.lidar_sensor=LidarSensor2(self.player,self.hud)
        if self.GS_camera is None: 
                self.GS_camera=self.Renderer(self.args)
    ...
    def record_video(self):
        if self.GS_image is None or self.lidar_sensor.range_image is  None: return
        rgb=self.GS_image.swapaxes(0, 2)
        rgb=cv2.cvtColor(rgb, cv2.COLOR_BGR2RGB)
        rgb=cv2.resize(rgb, dsize=(self.hud.dim[1],self.hud.dim[0]), interpolation = cv2.INTER_LINEAR)

        
        range=cv2.resize(self.lidar_sensor.range_image, dsize=(128,rgb.shape[0]))
        range=(range/256).astype('uint8')
        range = cv2.cvtColor(range, cv2.COLOR_GRAY2RGB)

        image = np.zeros((rgb.shape[0],rgb.shape[1]+128,3), np.uint8)
        image[:,0:-128]=rgb
        image[:,-128:]=range

        # Create a black image 
        # size = (image.shape[1]//4,image.shape[1]//4+20,3)
        # img = np.zeros(size, np.uint8)
        # pixels_per_meter=150
        # alpha=0.8
        # cv2.circle(img,(size[0]//2,size[0]//2),6,(255,0,0),-1) 
        # if self.waypoints is not None:
        #     for point in self.waypoints:
        #         center=(size[0]//2-int(point[1]*pixels_per_meter*4), int(size[0]//2-point[0]*pixels_per_meter))
        #         cv2.circle(img,center,3,(int(255*alpha),0,int(255*(1-alpha))),-1)
        #         alpha-=0.2
        # if self.info is not None:
        #     cv2.putText(img, "Speed: {:.2f}, Steer: {:.2f}".format(self.info[0], self.info[1]), (2,size[0]-10), cv2.FONT_HERSHEY_TRIPLEX, 0.5, (255,255,255))
        
        # image[-size[1]:,:size[0]]=img.swapaxes(0, 1)

        # Add the outside camera
        size = (image.shape[1]//4,image.shape[0]//4,3)
        camera=self.camera_manager._image.swapaxes(0, 1)
        camera=cv2.cvtColor(camera, cv2.COLOR_BGR2RGB)
        camera=cv2.resize(camera, dsize=(int(image.shape[0]/4), int(image.shape[1]/4)), interpolation = cv2.INTER_LINEAR)
        image[-size[1]:,:size[0]]=camera.swapaxes(0, 1)
        self.video.write(image.swapaxes(0, 1))
    ...
    def tick(self, clock, socket, control):
        if self.GS_camera is not None:
            ...
            T=transform[:3,3] 
            self.GS_image=self.GS_camera.render_view(R,T)
        self.hud.tick(self, clock)
        #Wait initialization of the sensors
        if self.GS_image is None or self.lidar_sensor.range_image is None:
            return True
        message={'end':False}
        ...
        image=self.GS_image.tobytes()
        range=self.lidar_sensor.range_image.tobytes()
        message["rgb_shape"]=self.GS_image.shape
        message["range_shape"]=self.lidar_sensor.range_image.shape
        message["speed"]=speed/MAX_SPEED
    ...
    def render(self, display):
        if self.show_lidar:
            ...
            if self.GS_image is not None:
                surface=pygame.surfarray.make_surface(self.GS_image.swapaxes(0,2))
                display.blit(surface, (0,0))
        self.camera_manager.render(display,small=True, offset4waypoints=True)
        self.lidar_sensor.render_range(display)
        if self.recording_enabled: self.record_video()
        self.hud.render(display)

    def write_output(self):
        position = self.player.get_transform().location
        rotation = self.player.get_transform().rotation
        timestamp = self.hud.simulation_time
        self.output_file.write("{},{},{},{},{},{},{},{}\n".format(timestamp,position.x, position.y, position.z,
                            math.radians(rotation.roll), math.radians(rotation.pitch), math.radians(rotation.yaw), self.status))
...
class JoystickControl(object):
    ...
    def check_near_collision(self, world : World, reverse=False): 
        if not RUMBLE or world.lidar_sensor.range_distance is None: return     
        if reverse:
            for col in itertools.chain(range(0,200), range(1848,2048)):
                for row in range(64,128):
                    if world.lidar_sensor.range_distance[row,col]<NEAR_COLLISION_DISTANCE:
    ...
    def parse_events(self, client, world:World, clock, sync_mode, socket_evaluator):
        ...
        for event in pygame.event.get():
            if event.type == pygame.JOYDEVICEADDED:
                ...
                if event.button == EXIT_BUTTON:
                    return True
                elif event.button == EXPERT_AGENT_BUTTON_EMERGENCY:
                    self.active = not self.active
                    if self.active:
                        socket_evaluator.send_string("emergency",zmq.NOBLOCK)
                        world.status=0
                    else: world.status=2
                    self.hud.notification('%r' % "Expert" if self.active else "Agent")
                elif event.button == EXPERT_AGENT_BUTTON_LOCAL_MINIMUM:
                    self.active = not self.active
                    if self.active:
                        socket_evaluator.send_string("local_minimum",zmq.NOBLOCK)
                        world.status=1
                    else: world.status=2
                    self.hud.notification('%r' % "Expert" if self.active else "Agent")
...
def render_waypoints(waypoints, display, height, width, info=None):
    # Create a black image
    size = (int(height/4), int(height/4),3)
    img = np.zeros(size, np.uint8)
    pixels_per_meter=75
    alpha=0.8
    ...
    if waypoints is not None:
        for point in waypoints:
            ...
            alpha-=0.2
    if info is not None:
        cv2.putText(img, "Speed: {:.2f}, Steer: {:.2f}".format(info[0], info[1]), (4,150), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255,255,255))
        if len(info)>2:
            cv2.putText(img, "Ctrl: {:.2f}, Ctrl: {:.2f}".format(info[2], info[3]), (4,160), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255,255,255))
            cv2.putText(img, "Traj: {:.2f}, Traj: {:.2f}".format(info[4], info[5]), (4,170), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (255,255,255))
    surface=pygame.surfarray.make_surface(img.swapaxes(0, 1))
    display.blit(surface, (width-size[0], 0))
...
def check_near_collision(depth, reverse=False):
        ...
        if reverse:
            for col in itertools.chain(range(0,250), range(1798,2048)):
                for row in range(64,128):
                    if depth[row, col]<min_distance:
                        min_distance=depth[row,col]
        else:
...
def game_loop(args):
    ...
    try:
        ...
        if args.output_dir is not None:
            ...
            world.output_file = open(args.output_dir, 'w')
        while True:
            if args.sync:
                sim_world.tick()
            clock.tick_busy_loop(60)
            
            if controller.parse_events(client, world, clock, args.sync, socket_evaluator):
                return
            ...
            if world.lidar_sensor.range_distance is not None:
                closest_distance= check_near_collision(world.lidar_sensor.range_distance,target_speed<0) 
            else: closest_distance=50.0
            ...
            render_waypoints(waypoints,display,args.height, args.width,info)
            pygame.display.flip()
            if args.output_dir is not None:
                world.write_output()
        ...
        if world.recording_enabled: 
            ...
            world.video.release()
        if args.output_dir is not None:
            print("Saving output file", args.output_dir)
...
def main():
    ...
    argparser.add_argument(
        ...
        metavar='H',
        default='gof',
        help='Gaussian splatting model (gof or hierarchical3DGS) (default: gof)')
    argparser.add_argument(
        '--scene',
        metavar='H',
        default='scene1',
    ...
    argparser.add_argument(
        ...
        metavar='H',
        default=None,
        help='Output directory of the data to evaluate trajectory. (it is not saved by default)')
